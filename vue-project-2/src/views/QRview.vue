...
watch(result, async () => {
  ...
  const userDoc = await getDoc(doc(db, `users/${result.value}`))
  if(!userDoc.exists()) {
    alert('Unable to create report with unknown user - Ensure the QR code was generated by MovieSim!')
    throw new Error(`Invalid userId - [${result.value}]`)
  }
  ...
  setDoc(doc(db, `users/${result.value}/reports/${userId.value}`), {
    name: currentUser.value.displayName.split(' ')[0] ?? userId.value,
    uid: userId.value,
  })
  alert('Created new report with ' + userDoc.data().name + '!')
})
...
function onDetect(detectedCodes) {
  try{
    ...
    alert('Unable to parse QR code - ensure it was generated by MovieSim!')
  }
}
