...
getUserSimilarities(props.current, props.compare, props.maxListings).then((simReport) => {
  ...
  zeroDiff.value = zeroDiff.value.map((item) => {
    return {
      // SHHH a little cheaty
      ...item,
      sim: 1,
    }
  })
...
<template>
  <transition name="expand">
    <div class="report-body" v-if="ready">
      ...
      <div v-if="zeroDiff && zeroDiff.length > 0">
        <label class="title">Movies You Both Watched</label>
        <hr />
        <MovieCarousel :list="zeroDiff"></MovieCarousel>
      </div>
